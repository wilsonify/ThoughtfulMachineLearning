"""
same as linear_regression2.py using minimize instead of solve
"""
import json
import logging
...
optimizers = ['Nelder-Mead', 'Powell', 'CG', 'BFGS',
...
class MyLinearRegression:
    ...
    def fit(self, x_input, y_input, with_intercept=True):
        ...
        def objective(betas):
            beta = np.array([[betas[0]], [betas[1]]])
            y_pred = x_input @ beta
            resid = y_input - y_pred
            ss = np.nansum(resid ** 2)
            return ss

        result = minimize(
            fun=objective,
            x0=np.array([20.9476, 1.0]),
            args=(),
            method=None,
            jac=None,
            hess=None,
        ...
        self.beta = np.array([[result.x[0]], [result.x[1]]])
